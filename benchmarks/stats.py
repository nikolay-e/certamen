...
def compute_cost_normalized_metrics(
    ...
    """
    acc_per_dollar = (
        accuracy / cost_dollars if cost_dollars > 0 else float("inf")
    )
    acc_per_minute = (
        accuracy / (duration_seconds / 60)
    ...
    return {
        ...
        "cost_per_correct": (
            cost_dollars / accuracy if accuracy > 0 else float("inf")
        ),
        "time_per_correct_seconds": (
            duration_seconds / accuracy if accuracy > 0 else float("inf")
        ),
    }
