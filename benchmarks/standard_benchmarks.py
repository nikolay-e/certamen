#!/usr/bin/env python3
"""
Standard Benchmark Evaluation: Certamen Framework on BBH & GPQA
...
Usage:
    python -m certamen.benchmarks.standard_benchmarks --config <path> --benchmark bbh
    python -m certamen.benchmarks.standard_benchmarks --config <path> --benchmark gpqa
    python -m certamen.benchmarks.standard_benchmarks --config <path> --benchmark both

Example:
    python -m certamen.benchmarks.standard_benchmarks --config config.benchmark.yml --benchmark bbh
"""
...
from certamen.models.base import LiteLLMModel
...
""".strip()
        ...
        try:
            ...
            predicted_answer = extract_answer(
                response_text, question_item["choices"]
            )
            ...
            logger.info(
                f"[{i}/{len(questions)}] {'✓' if is_correct else '✗'} {question_item['task']}"
            )
...
async def run_certamen_on_benchmark(
    questions: list[dict[str, Any]],
    config_path: str,
) -> dict[str, Any]:
    """Run Certamen Framework tournament on benchmark questions."""
    from datetime import datetime

    # Initialize Certamen using public API
    certamen = await Certamen.from_config(config_path=config_path)

    if not certamen.is_ready:
        raise RuntimeError(
            f"No healthy models available. Failed: {list(certamen.failed_models.keys())}"
        )
...
""".strip()
        ...
        try:
            ...
            response, metrics = await certamen.run_tournament(prompt)
            predicted_answer = extract_answer(
                response, question_item["choices"]
            )
            is_correct = predicted_answer == question_item["answer"]
            ...
            logger.info(
                f"[{i}/{len(questions)}] {'✓' if is_correct else '✗'} {question_item['task']} (Champion: {results[-1]['champion']})"
            )
...
def load_bbh_questions(num_per_task: int = 5) -> list[dict[str, Any]]:
                ...
                questions.append(
                    {
                        ...
                        "choices": example.get(
                            "choices", [example["target"]]
                        ),  # Some tasks have choices
                    }
                )
    ...
    logger.info(
        f"Loaded {len(questions)} questions from {len(BBH_TASKS)} BBH tasks"
    )
    return questions
...
async def run_benchmark_suite(
    ...
    print(
        "3. Run ablation studies (w/o Knowledge Bank, different judge models)"
    )
...
def main() -> None:
    ...
    if args.benchmark == "both":
        asyncio.run(
            run_benchmark_suite("bbh", config_path, args.num_questions)
        )
        asyncio.run(
            run_benchmark_suite("gpqa", config_path, args.num_questions)
        )
    else:
        asyncio.run(
            run_benchmark_suite(
