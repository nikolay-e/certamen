...
async def _run_single_models(
    certamen: Certamen, question: str
) -> list[dict[str, Any]]:
    results = []
    for model_name in certamen.healthy_models:
        ...
        print("=" * 80 + "\n")
        try:
            result = await run_single_model_with_cot(
...
def _print_cost_summary(
    ...
    print(f"  Certamen Tournament: ${tournament_result['cost_actual']:.4f}")
    avg_single_cost = sum(
...
def _write_micro_report(
    ...
    with open(output_path, "w") as f:
        ...
        f.write(
            f"Duration: {tournament_result['duration_seconds']:.1f} seconds "
            f"({tournament_result['duration_seconds'] / 60:.1f} minutes)\n\n"
        )
        ...
        if single_model_results:
            ...
            cost_multiple = (
                f"{cost_actual / cost_estimate:.1f}x"
                if cost_estimate > 0
                else "N/A (free/local baseline)"
            )
            f.write(f"Cost Multiple vs Single Model: {cost_multiple}\n\n")
            f.write(
                f"Time Multiple vs Single Model: "
                f"{tournament_result['duration_seconds'] / single_model_results[0]['duration_seconds']:.1f}x\n\n"
            )
...
async def main(args: dict[str, Any] | None = None) -> None:
    ...
    print("\n✅ Complete!")
    print(
        f"   Duration: {tournament_result['duration_seconds']:.1f}s "
        f"({tournament_result['duration_seconds'] / 60:.1f} min)"
    )
    ...
    _print_cost_summary(single_model_results, tournament_result)
