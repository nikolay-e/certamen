...
async def test_judge_fallback_to_emergency_judge(tmp_output_dir, mock_models):
    ...
    certamen._all_models["model_a"] = mock_models["model_a"]
    certamen._all_models["model_b"] = mock_models["model_b"]
    certamen._healthy_models["model_a"] = mock_models["model_a"]
    certamen._healthy_models["model_b"] = mock_models["model_b"]

    # Run tournament - should fallback when judge fails
    result, metrics = await certamen.run_tournament("What is 2+2?")
