...
class TestCertamenCreation:
    """Test creating Certamen instance from config."""

    @pytest.mark.asyncio
    async def test_create_certamen_from_config(
        self, basic_config: dict, tmp_output_dir: Path
    ) -> None:
        ...
        certamen = await app._create_certamen_from_config(
    ...
    @pytest.mark.asyncio
    async def test_certamen_outputs_dir_override(
        self, basic_config: dict, tmp_output_dir: Path
    ) -> None:
        ...
        config_obj = app._load_config(str(config_file))
        certamen = await app._try_create_certamen_from_config_obj(
            config_obj, skip_secrets=True
        )

        assert certamen.config_data["outputs_dir"] == cli_output_dir
...
class TestModelFiltering:
    ...
    def test_filter_requested_models_success(
        ...
        app.certamen = AsyncMock()
        app.certamen.healthy_models = {
        ...
        assert len(app.certamen._healthy_models) == 2
        assert "model_a" in app.certamen._healthy_models
        assert "model_b" in app.certamen._healthy_models
    ...
    def test_filter_requested_models_skipped_when_no_models_arg(
        ...
        assert len(app.certamen.healthy_models) == 2


class TestCertamenValidation:
    """Test Certamen readiness validation."""

    def test_validate_certamen_ready_success(self) -> None:
        """Test validation passes when Certamen is ready."""
        args = {
            "config": "config.yml",
            "outputs_dir": None,
            "no_secrets": False,
        ...
        app.certamen = AsyncMock()
        app.certamen.is_ready = True

        app._validate_certamen_ready()

    def test_validate_certamen_not_initialized(self) -> None:
        """Test validation fails when Certamen not initialized."""
        args = {
            "config": "config.yml",
            "outputs_dir": None,
            "no_secrets": False,
    ...
    def test_validate_certamen_not_ready(self) -> None:
...
class TestCliWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_initialize_certamen_success(
        self, basic_config: dict, tmp_output_dir: Path
    ) -> None:
        ...
        assert app.certamen is not None
        assert app.certamen.is_ready

    @pytest.mark.asyncio
    async def test_initialize_certamen_with_missing_config(self) -> None:
    ...
    async def test_full_cli_flow_with_mock_models(
        ...
        result, metrics = await app.certamen.run_tournament(question)
...
class TestAppRunMethod:
    ...
    async def test_run_with_keyboard_interrupt(
        ...
        app.certamen = AsyncMock()
        app.certamen.run_tournament = AsyncMock(
