...
class TestJudgeMode:
    ...
    async def test_tournament_with_judge_model(
        ...
        certamen._healthy_models = mock_models
        certamen._comparison = certamen._create_comparison()

        # Judge should be removed from active participants
        assert "judge_model" not in certamen._comparison.active_model_keys
        assert len(certamen._comparison.active_model_keys) == 2
    ...
    async def test_judge_scores_used_for_elimination(
        ...
        comparison = certamen._comparison
...
class TestEmergencyJudgeFallback:
    ...
    async def test_judge_model_error_handling(
        ...
        result = await certamen._comparison.run_cross_evaluation(
            "Test question?",
            {"LLM1": "Response A", "LLM2": "Response B"},
            round_num=1,
        )
        ...
        assert certamen._comparison.evaluation_scores != {}
...
class TestKnowledgeBankIntegration:
    ...
    async def test_eliminated_model_insights_extracted(
        ...
        assert certamen._comparison.knowledge_bank is not None
        ...
        kb = certamen._comparison.knowledge_bank
