...
class TestKnowledgeBankExtraction:
    ...
    async def test_knowledge_bank_extracts_insights_from_eliminated_model(
        ...
        comparison = certamen._last_comparison
...
class TestKnowledgeBankIntegration:
    ...
    async def test_tournament_with_kb_vs_without_kb(
        ...
        certamen_no_kb = await Certamen.from_settings(
            settings=basic_config,
            skip_secrets=True,
        )
        ...
        certamen_no_kb._healthy_models = mock_models_1

        result_no_kb, metrics_no_kb = await certamen_no_kb.run_tournament(
            "Test question?"
        )

        # Access the comparison object that was used
        comparison_no_kb = certamen_no_kb._last_comparison
        ...
        certamen_with_kb = await Certamen.from_settings(
            settings=basic_config,
            skip_secrets=True,
        )
        ...
        certamen_with_kb._healthy_models = mock_models_2

        (
            result_with_kb,
            metrics_with_kb,
        ) = await certamen_with_kb.run_tournament("Test question?")

        # Access the comparison object that was used
        comparison_with_kb = certamen_with_kb._last_comparison
