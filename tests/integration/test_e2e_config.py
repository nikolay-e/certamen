...
class TestConfigurationLoading:
    ...
    async def test_load_config_from_file(
        ...
        assert "models" in certamen.config_data
        assert "retry" in certamen.config_data
        assert "features" in certamen.config_data
    ...
    async def test_load_config_from_settings_dict(
        ...
        # Verify config loaded
        assert certamen.config_data is not None
        assert certamen.config_data["models"] == basic_config["models"]
...
class TestConfigurationMerging:
    ...
    async def test_defaults_fill_missing_values(
        ...
        assert "retry" in certamen.config_data
        assert "max_attempts" in certamen.config_data["retry"]

        # Should have default features
        assert "features" in certamen.config_data

        # Should have default prompts
        assert "prompts" in certamen.config_data
    ...
    async def test_only_specified_models_are_loaded(
        ...
        model_keys = certamen.config.get_active_model_keys()
...
class TestModelInitialization:
    ...
    async def test_models_are_created_from_config(
        ...
        assert len(certamen.all_models) == len(basic_config["models"])
    ...
    async def test_model_properties_are_set_correctly(
        ...
        # Get the model
        model = certamen.all_models["test_model"]
...
class TestHealthChecks:
    ...
    async def test_healthy_models_pass_health_check(
        ...
        assert len(certamen.healthy_models) > 0
    ...
    async def test_failed_models_tracked_separately(
        ...
        # Create certamen without health check first
        certamen = await Certamen.from_settings(
            settings=basic_config,
            skip_secrets=True,
            skip_health_check=True,
        ...
        test_models = {
            "healthy": certamen.all_models["model_a"],
    ...
    async def test_skip_health_check_flag_works(
        ...
        assert len(certamen.healthy_models) == len(certamen.all_models)
        assert len(certamen.failed_models) == 0
    ...
    async def test_is_ready_property(
        ...
        assert certamen.is_ready
        assert certamen.healthy_model_count > 0
...
class TestOutputsDirectory:
    ...
    async def test_outputs_dir_can_be_relative_or_absolute(
        ...
        certamen1 = await Certamen.from_settings(
            settings=basic_config,
            skip_secrets=True,
            skip_health_check=True,
        )
        assert certamen1 is not None

        # Test absolute path
        with tempfile.TemporaryDirectory() as tmpdir:
            basic_config["outputs_dir"] = tmpdir
            certamen2 = await Certamen.from_settings(
                settings=basic_config,
                skip_secrets=True,
                skip_health_check=True,
            )
            assert certamen2 is not None

    @pytest.mark.asyncio
    async def test_outputs_dir_is_accessible_from_certamen(
        self,
        basic_config: dict,
    ) -> None:
        """Test that outputs_dir is accessible from Certamen instance."""
        certamen = await Certamen.from_settings(
            settings=basic_config,
            skip_secrets=True,
            skip_health_check=True,
        ...
        assert "outputs_dir" in certamen.config_data
        assert certamen.config_data["outputs_dir"] is not None
...
class TestConfigurationEdgeCases:
    ...
    async def test_config_deep_merge_preserves_nested_values(
        ...
        retry_config = certamen.config_data["retry"]
