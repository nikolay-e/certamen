...
from certamen_core.infrastructure.config.models import (
    CertamenConfig,
    ModelConfig,
)
from certamen_core.shared.logging import get_contextual_logger
from certamen_core.shared.mapping_utils import deep_merge
...
def validate_config(config_data: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    logger.debug(
        "Config validation: is_valid=%s, errors=%s", is_valid, len(errors)
    )
...
class Config:
    ...
    def _load_defaults(self) -> dict[str, Any]:
        ...
        logger.debug(
            "Loaded defaults with sections: %s", list(defaults.keys())
        )
        return defaults
    ...
    def _validate_config_path(self) -> bool:
        ...
        if not self.config_path.exists():
            logger.error(
                "Config file not found at %s", self.config_path.resolve()
            )
            return False
        return True
    ...
    def _merge_single_model(
        ...
        if not base_model:
            logger.warning(
                "Model '%s' not found in defaults. "
                "Provide full configuration (provider, model_name, etc.)",
                model_key,
            )
    ...
    def _build_filtered_models(
        ...
        logger.debug("User specified models: %s", user_model_keys)
    ...
    def load(self) -> bool:
        try:
            ...
            logger.critical(
                "Unexpected error loading config: %s", e, exc_info=True
            )
            return False
