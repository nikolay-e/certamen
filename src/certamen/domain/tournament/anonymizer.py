...
from certamen.shared.constants import ANONYMOUS_MODEL_PREFIX
...
class ModelAnonymizer:
    def __init__(self, deterministic_mode: bool = False):
        self.logger = get_contextual_logger("certamen.anonymizer")
    ...
    def anonymize_responses(
        ...
        for real_name, anon_name in forward_mapping.items():
            self.logger.debug("  %s → %s", real_name, anon_name)
