...
from certamen.domain.workflow import BaseNode as DomainBaseNode
from certamen.domain.workflow import (
    ExecutionContext,
    NodeProperty,
    Port,
    PortType,
...
async def safe_generate(model: Any, prompt: str) -> tuple[str, bool]:
    try:
        ...
        logger.error("Exception during model generation: %s", e, exc_info=True)
        return "", False
