"""LiteLLM model implementation for Certamen Framework."""
...
from ..config.env_utils import get_ollama_base_url
from ..logging import get_contextual_logger
from ..utils.exceptions import ExceptionClassifier, ModelResponseError
from ..utils.json_ import to_dict
    ...
    def _auto_detect_max_tokens(
        ...
        if (
            ...
            if max_output_tokens:
                ...
                default_max_tokens = min(4096, int(context_win * 0.25))
                logger.warning(
                    f"max_tokens not provided for {model_key} and could not be auto-detected. "
                    f"Using default value: {default_max_tokens} (25% of context window)"
                )
                model_config["max_tokens"] = default_max_tokens
