"""Command-line argument handling for Certamen Framework."""
...
def _add_gui_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help=f"Host to bind to (default: {DEFAULT_GUI_HOST})",
    )
    parser.add_argument(
        "--port",
        type=int,
        default=DEFAULT_GUI_PORT,
        help=f"Port to bind to (default: {DEFAULT_GUI_PORT})",
    )
...
def parse_arguments() -> dict[str, Any]:
    parser = argparse.ArgumentParser(
        description="Certamen Framework - LLM Comparison and Evaluation Tool"
    )

    parser.add_argument(
        "--version",
        action="version",
        version=f"Certamen Framework {__version__}",
    )
    ...
    gui_parser = subparsers.add_parser(
        "gui",
        help="Start the visual workflow editor",
    )
    _add_common_args(gui_parser)
    _add_gui_args(gui_parser)
