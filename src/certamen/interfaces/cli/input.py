...
logger = get_contextual_logger("certamen.interfaces.cli.input")
...
def _try_set_future_exception(
    ...
    try:
        ...
        logger.debug(
            "Input future already done when exception occurred: %s", exception
        )
...
async def _get_input_with_validation(
    ...
    while True:
        try:
            ...
            from certamen.domain.errors import InputError
...
async def async_input(
    prompt: str = "",
    default: str = "",
    timeout: int = DEFAULT_INPUT_TIMEOUT,  # caller-provided timeout for interactive CLI
