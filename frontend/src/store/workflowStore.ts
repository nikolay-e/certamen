...
function restoreNodeDynamicPorts(
  node: WorkflowNode,
  enrichedEdges: Edge[],
): { updatedNode: WorkflowNode; needsUpdate: boolean } {
  if (!node.data.dynamicInputs) {
    return { updatedNode: node, needsUpdate: false };
  }

  const { prefix, port_type, min_count } = node.data.dynamicInputs;
  const oldStyleRe = new RegExp(String.raw`^${prefix}\d+$`);
  const newStyleExtractRe = new RegExp(String.raw`^${prefix}_(\d+)$`);
  const oldStyleExtractRe = new RegExp(String.raw`^${prefix}(\d+)$`);
  ...
  if (targetPortCount > currentMax) {
    ...
    return {
      updatedNode: { ...node, data: { ...node.data, inputs: newInputs } },
      needsUpdate: true,
    };
  }
